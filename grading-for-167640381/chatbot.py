    ...
    def process(self, line):
        if self.creative:
            ...
            if self.SPELL_CHECKING == 'OFF' and l in ["yes", "y", "yeah", "yup" , "yeh"] and self.count == 5:
                response = "Make a recommendation, I can! 'All right, enough for me that is. Recommend, I do '" + self.indices_to_titles[self.rec[self.index]] + ".' Another recommendation, you want to hear? Say 'yes', if one you desire. If done, enter ':quit'."
                return response
...
Before using:
    ...
    - Emailed Prof. Jurafsky - given no response, we instead checked with OH staff (next item)
